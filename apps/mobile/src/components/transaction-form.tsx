...
export function TransactionForm({ existing }: { existing?: Transaction }) {
  ...
  async function persistDraft() {
    ...
    try { await saveDraft(user.id, family.id, { type, date, amount, account, category, description, remarks, key }); setDirty(false); Alert.alert('Draft saved', 'It stays on this device until saved as a transaction or you sign out.'); } catch { setError('Unable to save draft on this device.'); }
  }
  async function restoreDraft() {
    if (!user || !family) return;
    try { const draft = await loadDraft(user.id, family.id); if (!draft) { setError('There is no saved draft for this family.'); return; } setType(draft.type); setDate(draft.date); setAmount(draft.amount); setAccount(draft.account); setCategory(draft.category); setDescription(draft.description); setRemarks(draft.remarks); setKey(draft.key); setDirty(true); } catch { setError('Unable to restore draft.'); }
  }
  ...
  async function save() {
    ...
    try {
      ...
      if (!account || !category) throw new Error('Choose an account and category.');
      await rpc('save_transaction', { p_family_id: family.id, p_transaction_id: existing?.id ?? null, p_expected_version: existing?.version ?? null, p_type: type, p_local_date: date, p_amount_minor: parseMoney(amount), p_account_id: account, p_category_id: category, p_description: description, p_remarks: remarks, p_idempotency_key: key });
